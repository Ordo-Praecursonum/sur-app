...
//
//  FIXED: Proper error handling for Secp256k1.addModN which now returns optional Data?
//
...
final class EthereumKeyManager {
    ...
    private static func deriveChildKey(
        ...
        // CRITICAL FIX: Secp256k1.addModN now returns Data? so we must unwrap it
        guard let childKey = addPrivateKeys(parentPrivateKey, keyData) else {
            // If addModN returns nil, it means the derivation produced an invalid key (zero or >= n)
            // According to BIP-32 spec, we should try the next index, but at this level we just fail
            throw EthereumKeyError.derivationFailed
        }
        
        // Double-check validity (addModN should already ensure this)
    ...
    ///
    /// FIXED: Now properly handles the optional return from Secp256k1.addModN
    ///
    /// - Parameters:
    ///   - key1: First private key
    ///   - key2: Second private key
    /// - Returns: Sum modulo curve order, or nil if result is invalid
    private static func addPrivateKeys(_ key1: Data, _ key2: Data) -> Data? {
        // Secp256k1.addModN returns Data? (optional) - it returns nil if the result would be invalid
        // This can happen if the sum equals zero or is >= curve order (extremely rare)
